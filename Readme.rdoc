...
the developer program at the {Ponoko website}[]
...
API eg, get_material_catlogue, get_products, post_order etc. These methods return hashes 
...
High level API calls will only make calls to the Ponoko servers only if necessary, ie.
the information hasn't already been fetched. Though you can force a fecth from the servers
by using the bang ('!') versions of the calls.
